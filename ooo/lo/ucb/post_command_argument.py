...
class PostCommandArgument(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``PostCommandArgument`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``PostCommandArgument``
        ...
        key_order = ('Source', 'Sink')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], PostCommandArgument):
                oth = args[0]
        ...
        if  arg_len > len(key_order):
            raise ValueError("PostCommandArgument.__init__() To many parameters")
