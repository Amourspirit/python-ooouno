...
if not TYPE_CHECKING and _DYNAMIC:
    def _dynamic_ex() -> None:
        import uno
        # Dynamically create uno com.sun.star.ucb.UnsupportedDataSinkException using uno
        global UnsupportedDataSinkException

        def _set_fn_attr(ex):
            type_name = 'com.sun.star.ucb.UnsupportedDataSinkException'
        ...
        def _set_attr(ex):
            ...
            ex.__dict__['__ooo_full_ns__'] = 'com.sun.star.ucb.UnsupportedDataSinkException'
            ex.__dict__['__ooo_type_name__'] = 'exception'

        def _ex_init(Sink = UNO_NONE, **kwargs):
            ns = 'com.sun.star.ucb.UnsupportedDataSinkException'
            ex = uno.createUnoStruct(ns)
            if not Sink is UNO_NONE:
                if getattr(ex, 'Sink') != Sink:
                    setattr(ex, 'Sink', Sink)
        ...
        UnsupportedDataSinkException = _ex_init
