...
class SocketPermission(object):
    ...
    **since**
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``SocketPermission`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``SocketPermission``
        ...
        key_order = ('Host', 'Actions')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], SocketPermission):
                oth = args[0]
        ...
        if  arg_len > len(key_order):
            raise ValueError("SocketPermission.__init__() To many parameters")
