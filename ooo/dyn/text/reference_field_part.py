...
if not TYPE_CHECKING and _DYNAMIC:
    from com.sun.star.text import ReferenceFieldPart as ReferenceFieldPart
    if hasattr(ReferenceFieldPart, '_constants') and isinstance(ReferenceFieldPart._constants, dict):
        ReferenceFieldPart._constants['__ooo_ns__'] = 'com.sun.star.text'
        ReferenceFieldPart._constants['__ooo_full_ns__'] = 'com.sun.star.text.ReferenceFieldPart'
        ReferenceFieldPart._constants['__ooo_type_name__'] = 'const'
    def build_enum():
        global ReferenceFieldPartEnum
        ls = [f for f in dir(ReferenceFieldPart) if not callable(getattr(ReferenceFieldPart, f)) and not f.startswith('__')]
        _dict = {}
        for name in ls:
            _dict[name] = getattr(ReferenceFieldPart, name)
        ReferenceFieldPartEnum = IntEnum('ReferenceFieldPartEnum', _dict)
    build_enum()
else:
