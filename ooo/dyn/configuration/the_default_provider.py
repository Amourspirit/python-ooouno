...
if not TYPE_CHECKING and _DYNAMIC:
    def _dynamic_singleton() -> None:
        ...
        import uno
        global theDefaultProvider

        def _singleton_init():
            ctx = uno.getComponentContext()
            key = '/singletons/com.sun.star.configuration.theDefaultProvider/service'
            ...
            if not hasattr(singleton, '__ooo_ns__'):
                setattr(singleton, ' __ooo_ns__', 'com.sun.star.configuration')
                setattr(singleton, ' __ooo_full_ns__', 'com.sun.star.configuration.theDefaultProvider')
                setattr(singleton, ' __ooo_type_name__', 'singleton')
            return singleton
        theDefaultProvider = _singleton_init
    _dynamic_singleton()
else:
