...
if (not TYPE_CHECKING) and UNO_RUNTIME and UNO_ENVIRONMENT:
    ...
    def _get_class():
        orig_init = None
        def init(self, package = UNO_NONE, **kwargs):
            if getattr(package, "__class__", None) == self.__class__:
                orig_init(self, package)
                return
            else:
                orig_init(self)
            if not package is UNO_NONE:
                if getattr(self, 'package') != package:
                    setattr(self, 'package', package)
