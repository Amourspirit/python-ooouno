...
class GraphicCrop(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``GraphicCrop`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``GraphicCrop``
        ...
        key_order = ('Top', 'Bottom', 'Left', 'Right')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], GraphicCrop):
                oth = args[0]
        ...
        if  arg_len > len(key_order):
            raise ValueError("GraphicCrop.__init__() To many parameters")
