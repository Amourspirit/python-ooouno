...
if (not TYPE_CHECKING) and UNO_RUNTIME and UNO_ENVIRONMENT:
    ...
    def _get_class():
        orig_init = None
        def init(self, Coordinates = UNO_NONE, Flags = UNO_NONE):
            if getattr(Coordinates, "__class__", None) == self.__class__:
                orig_init(self, Coordinates)
                return
            else:
                orig_init(self)
            if not Coordinates is UNO_NONE:
                if getattr(self, 'Coordinates') != Coordinates:
    ...
    PolyPolygonBezierCoords = _get_class()
