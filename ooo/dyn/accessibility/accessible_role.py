...
if not TYPE_CHECKING and _DYNAMIC:
    ...
    if hasattr(AccessibleRole, '_constants') and isinstance(AccessibleRole._constants, dict):
        AccessibleRole._constants['__ooo_ns__'] = 'com.sun.star.accessibility'
        AccessibleRole._constants['__ooo_full_ns__'] = 'com.sun.star.accessibility.AccessibleRole'
        AccessibleRole._constants['__ooo_type_name__'] = 'const'
    def build_enum():
        global AccessibleRoleEnum
        ls = [f for f in dir(AccessibleRole) if not callable(getattr(AccessibleRole, f)) and not f.startswith('__')]
        _dict = {}
        for name in ls:
            _dict[name] = getattr(AccessibleRole, name)
        AccessibleRoleEnum = IntEnum('AccessibleRoleEnum', _dict)
    build_enum()
else:
