...
if not TYPE_CHECKING and _DYNAMIC:
    ...
    if hasattr(ContentAction, '_constants') and isinstance(ContentAction._constants, dict):
        ContentAction._constants['__ooo_ns__'] = 'com.sun.star.ucb'
        ContentAction._constants['__ooo_full_ns__'] = 'com.sun.star.ucb.ContentAction'
        ContentAction._constants['__ooo_type_name__'] = 'const'
    def build_enum():
        global ContentActionEnum
        ls = [f for f in dir(ContentAction) if not callable(getattr(ContentAction, f)) and not f.startswith('__')]
        _dict = {}
        for name in ls:
            _dict[name] = getattr(ContentAction, name)
        ContentActionEnum = IntFlag('ContentActionEnum', _dict)
    build_enum()
else:
