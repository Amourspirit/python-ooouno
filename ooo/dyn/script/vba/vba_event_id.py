...
if not TYPE_CHECKING and _DYNAMIC:
    ...
    if hasattr(VBAEventId, '_constants') and isinstance(VBAEventId._constants, dict):
        VBAEventId._constants['__ooo_ns__'] = 'com.sun.star.script.vba'
        VBAEventId._constants['__ooo_full_ns__'] = 'com.sun.star.script.vba.VBAEventId'
        VBAEventId._constants['__ooo_type_name__'] = 'const'
    def build_enum():
        global VBAEventIdEnum
        ls = [f for f in dir(VBAEventId) if not callable(getattr(VBAEventId, f)) and not f.startswith('__')]
        _dict = {}
        for name in ls:
            _dict[name] = getattr(VBAEventId, name)
        VBAEventIdEnum = IntEnum('VBAEventIdEnum', _dict)
    build_enum()
else:
