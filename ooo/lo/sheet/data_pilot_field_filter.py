...
class DataPilotFieldFilter(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``DataPilotFieldFilter`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``DataPilotFieldFilter``
        ...
        self._match_value_name = None
        self._match_value = None

        key_order = ('FieldName', 'MatchValueName', 'MatchValue')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], DataPilotFieldFilter):
                oth = args[0]
