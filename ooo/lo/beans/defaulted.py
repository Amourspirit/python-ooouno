...
class Defaulted(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``Defaulted`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``Defaulted``
        ...
        """
        self._value = None
        self._is_defaulted = None

        key_order = ('Value', 'IsDefaulted')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], Defaulted):
                oth = args[0]
