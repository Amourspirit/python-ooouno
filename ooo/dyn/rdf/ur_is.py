...
if not TYPE_CHECKING and _DYNAMIC:
    from com.sun.star.rdf import URIs as URIs
    if hasattr(URIs, '_constants') and isinstance(URIs._constants, dict):
        URIs._constants['__ooo_ns__'] = 'com.sun.star.rdf'
        URIs._constants['__ooo_full_ns__'] = 'com.sun.star.rdf.URIs'
        URIs._constants['__ooo_type_name__'] = 'const'
    def build_enum():
        global URIsEnum
        ls = [f for f in dir(URIs) if not callable(getattr(URIs, f)) and not f.startswith('__')]
        _dict = {}
        for name in ls:
            _dict[name] = getattr(URIs, name)
        URIsEnum = IntEnum('URIsEnum', _dict)
    build_enum()
else:
