...
class PropertyValueInfo(PropertyValue_c9610c73):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``PropertyValueInfo`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``PropertyValueInfo``
        ...
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], PropertyValueInfo):
                oth = args[0]
