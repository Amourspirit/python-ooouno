...
if (not TYPE_CHECKING) and UNO_RUNTIME and UNO_ENVIRONMENT:
    ...
    def _get_class():
        orig_init = None
        def init(self, DialogResult = UNO_NONE, **kwargs):
            if getattr(DialogResult, "__class__", None) == self.__class__:
                orig_init(self, DialogResult)
                return
            else:
                orig_init(self)
            if not DialogResult is UNO_NONE:
                if getattr(self, 'DialogResult') != DialogResult:
                    setattr(self, 'DialogResult', DialogResult)
