...
if (not TYPE_CHECKING) and UNO_RUNTIME and UNO_ENVIRONMENT:
    ...
    def _get_class():
        orig_init = None
        def init(self, invalidProperty = UNO_NONE, reason = UNO_NONE, **kwargs):
            if getattr(invalidProperty, "__class__", None) == self.__class__:
                orig_init(self, invalidProperty)
                return
            else:
                orig_init(self)
            if not invalidProperty is UNO_NONE:
                if getattr(self, 'invalidProperty') != invalidProperty:
                    setattr(self, 'invalidProperty', invalidProperty)
        ...
        ex.__ooo_ns__ = 'com.sun.star.bridge'
