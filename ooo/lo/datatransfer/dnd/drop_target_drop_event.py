...
class DropTargetDropEvent(DropTargetEvent_8d651169):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``DropTargetDropEvent`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``DropTargetDropEvent``
        ...
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], DropTargetDropEvent):
                oth = args[0]
