...
class SetPropertyTolerantFailed(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``SetPropertyTolerantFailed`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``SetPropertyTolerantFailed``
        ...
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], SetPropertyTolerantFailed):
                oth = args[0]
