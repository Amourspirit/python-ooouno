...
class ContextMenuExecuteEvent(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``ContextMenuExecuteEvent`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``ContextMenuExecuteEvent``
        ...
        self._execute_position = None
        self._action_trigger_container = None
        self._selection = None

        key_order = ('SourceWindow', 'ExecutePosition', 'ActionTriggerContainer', 'Selection')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], ContextMenuExecuteEvent):
                oth = args[0]
        ...
        if  arg_len > len(key_order):
            raise ValueError("ContextMenuExecuteEvent.__init__() To many parameters")
