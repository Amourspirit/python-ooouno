...
class ListAction(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``ListAction`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``ListAction``
        ...
        self._position = None
        self._count = None
        self._list_action_type = None
        self._action_info = None

        key_order = ('Position', 'Count', 'ListActionType', 'ActionInfo')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], ListAction):
                oth = args[0]
        ...
        if  arg_len > len(key_order):
            raise ValueError("ListAction.__init__() To many parameters")
