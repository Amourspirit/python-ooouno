...
class TabPageActivatedEvent(EventObject_a3d70b03):
    ...
    **since**
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``TabPageActivatedEvent`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``TabPageActivatedEvent``
                values. Following the order of ``Keyword Arguments``.

        Keyword Arguments:
            TabPageID (int, optional): TabPageID value
        """
        self._tab_page_id = None

        key_order = ('TabPageID',)
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], TabPageActivatedEvent):
                oth = args[0]
