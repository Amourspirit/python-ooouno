...
class SystemDependentXWindow(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``SystemDependentXWindow`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``SystemDependentXWindow``
        ...
        self._display_pointer = None

        key_order = ('WindowHandle', 'DisplayPointer')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], SystemDependentXWindow):
                oth = args[0]
