...
if (not TYPE_CHECKING) and UNO_RUNTIME and UNO_ENVIRONMENT:
    ...
    def _get_class():
        orig_init = None
        def init(self, VerbId = UNO_NONE, VerbName = UNO_NONE, VerbIsOnMenu = UNO_NONE, VerbIsConst = UNO_NONE):
            ...
            if not VerbIsOnMenu is UNO_NONE:
                if getattr(self, 'VerbIsOnMenu') != VerbIsOnMenu:
                    setattr(self, 'VerbIsOnMenu', VerbIsOnMenu)
            if not VerbIsConst is UNO_NONE:
                if getattr(self, 'VerbIsConst') != VerbIsConst:
                    setattr(self, 'VerbIsConst', VerbIsConst)

        type_name = 'com.sun.star.frame.status.Verb'
