...
if (not TYPE_CHECKING) and UNO_RUNTIME and UNO_ENVIRONMENT:
    ...
    def _get_class():
        orig_init = None
        def init(self, FeatureURL = UNO_NONE, FrameName = UNO_NONE, SearchFlags = UNO_NONE):
            ...
            if not FrameName is UNO_NONE:
                if getattr(self, 'FrameName') != FrameName:
                    setattr(self, 'FrameName', FrameName)
            if not SearchFlags is UNO_NONE:
                if getattr(self, 'SearchFlags') != SearchFlags:
                    setattr(self, 'SearchFlags', SearchFlags)
