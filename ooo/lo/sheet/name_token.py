...
class NameToken(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``NameToken`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``NameToken``
        ...
        key_order = ('Index', 'Sheet')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], NameToken):
                oth = args[0]
        ...
        if  arg_len > len(key_order):
            raise ValueError("NameToken.__init__() To many parameters")
