...
if not TYPE_CHECKING and _DYNAMIC:
    ...
    if hasattr(InterpolationMode, '_constants') and isinstance(InterpolationMode._constants, dict):
        InterpolationMode._constants['__ooo_ns__'] = 'com.sun.star.rendering'
        InterpolationMode._constants['__ooo_full_ns__'] = 'com.sun.star.rendering.InterpolationMode'
        InterpolationMode._constants['__ooo_type_name__'] = 'const'
    def build_enum():
        global InterpolationModeEnum
        ls = [f for f in dir(InterpolationMode) if not callable(getattr(InterpolationMode, f)) and not f.startswith('__')]
        _dict = {}
        for name in ls:
            _dict[name] = getattr(InterpolationMode, name)
        InterpolationModeEnum = IntEnum('InterpolationModeEnum', _dict)
    build_enum()
else:
