...
if (not TYPE_CHECKING) and UNO_RUNTIME and UNO_ENVIRONMENT:
    ...
    def _get_class():
        orig_init = None
        def init(self, Component = UNO_NONE, **kwargs):
            if getattr(Component, "__class__", None) == self.__class__:
                orig_init(self, Component)
                return
            else:
                orig_init(self)
            if not Component is UNO_NONE:
                if getattr(self, 'Component') != Component:
    ...
    ComponentChangeEvent = _get_class()
