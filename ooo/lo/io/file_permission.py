...
class FilePermission(object):
    ...
    **since**
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``FilePermission`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``FilePermission``
        ...
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], FilePermission):
                oth = args[0]
