...
class VerbDescriptor(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``VerbDescriptor`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``VerbDescriptor``
        ...
        self._verb_flags = None
        self._verb_attributes = None

        key_order = ('VerbID', 'VerbName', 'VerbFlags', 'VerbAttributes')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], VerbDescriptor):
                oth = args[0]
        ...
        if  arg_len > len(key_order):
            raise ValueError("VerbDescriptor.__init__() To many parameters")
