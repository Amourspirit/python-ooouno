...
class TextPosition(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``TextPosition`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``TextPosition``
        ...
        self._position_in_paragraph = None

        key_order = ('Paragraph', 'PositionInParagraph')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], TextPosition):
                oth = args[0]
