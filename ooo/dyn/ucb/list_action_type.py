...
if not TYPE_CHECKING and _DYNAMIC:
    ...
    if hasattr(ListActionType, '_constants') and isinstance(ListActionType._constants, dict):
        ListActionType._constants['__ooo_ns__'] = 'com.sun.star.ucb'
        ListActionType._constants['__ooo_full_ns__'] = 'com.sun.star.ucb.ListActionType'
        ListActionType._constants['__ooo_type_name__'] = 'const'
    def build_enum():
        global ListActionTypeEnum
        ls = [f for f in dir(ListActionType) if not callable(getattr(ListActionType, f)) and not f.startswith('__')]
        _dict = {}
        for name in ls:
            _dict[name] = getattr(ListActionType, name)
        ListActionTypeEnum = IntEnum('ListActionTypeEnum', _dict)
    build_enum()
else:
