...
from .enum_type_description_comp import EnumTypeDescriptionComp
...
class EnumHelper:
    ...
    def get_enum_info(name: str) -> EnumTypeDescriptionComp | None:
        ...
        if odt := cast(XEnumTypeDescription, Reflect.get_const_info(name)):
            return EnumTypeDescriptionComp(odt) if uno_helper.is_uno_interfaces(odt, XEnumTypeDescription) else None
        else:
            return None
