...
if not TYPE_CHECKING and _DYNAMIC:
    ...
    if hasattr(LocaleItem, '_constants') and isinstance(LocaleItem._constants, dict):
        LocaleItem._constants['__ooo_ns__'] = 'com.sun.star.i18n'
        LocaleItem._constants['__ooo_full_ns__'] = 'com.sun.star.i18n.LocaleItem'
        LocaleItem._constants['__ooo_type_name__'] = 'const'
    def build_enum():
        global LocaleItemEnum
        ls = [f for f in dir(LocaleItem) if not callable(getattr(LocaleItem, f)) and not f.startswith('__')]
        _dict = {}
        for name in ls:
            _dict[name] = getattr(LocaleItem, name)
        LocaleItemEnum = IntEnum('LocaleItemEnum', _dict)
    build_enum()
else:
