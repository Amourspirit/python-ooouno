...
if (not TYPE_CHECKING) and UNO_RUNTIME and UNO_ENVIRONMENT:
    ...
    def _get_class():
        orig_init = None
        def init(self, Series = UNO_NONE, Categories = UNO_NONE):
            if getattr(Series, "__class__", None) == self.__class__:
                orig_init(self, Series)
                return
            else:
                orig_init(self)
            if not Series is UNO_NONE:
                if getattr(self, 'Series') != Series:
