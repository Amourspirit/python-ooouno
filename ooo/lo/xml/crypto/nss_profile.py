...
class NSSProfile(object):
    ...
    **since**
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``NSSProfile`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``NSSProfile``
        ...
        key_order = ('Name', 'Path', 'Type')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], NSSProfile):
                oth = args[0]
        ...
        if  arg_len > len(key_order):
            raise ValueError("NSSProfile.__init__() To many parameters")
