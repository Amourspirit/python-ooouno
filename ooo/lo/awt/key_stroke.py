...
class KeyStroke(object):
    ...
    **since**
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``KeyStroke`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``KeyStroke``
        ...
        key_order = ('Modifiers', 'KeyCode', 'KeyChar', 'KeyFunc')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], KeyStroke):
                oth = args[0]
        ...
        if  arg_len > len(key_order):
            raise ValueError("KeyStroke.__init__() To many parameters")
