...
class ParagraphTarget(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``ParagraphTarget`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``ParagraphTarget``
        ...
        key_order = ('Shape', 'Paragraph')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], ParagraphTarget):
                oth = args[0]
        ...
        if  arg_len > len(key_order):
            raise ValueError("ParagraphTarget.__init__() To many parameters")
