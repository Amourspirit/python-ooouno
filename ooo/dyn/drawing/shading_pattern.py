...
if not TYPE_CHECKING and _DYNAMIC:
    ...
    if hasattr(ShadingPattern, '_constants') and isinstance(ShadingPattern._constants, dict):
        ShadingPattern._constants['__ooo_ns__'] = 'com.sun.star.drawing'
        ShadingPattern._constants['__ooo_full_ns__'] = 'com.sun.star.drawing.ShadingPattern'
        ShadingPattern._constants['__ooo_type_name__'] = 'const'
    def build_enum():
        global ShadingPatternEnum
        ls = [f for f in dir(ShadingPattern) if not callable(getattr(ShadingPattern, f)) and not f.startswith('__')]
        _dict = {}
        for name in ls:
            _dict[name] = getattr(ShadingPattern, name)
        ShadingPatternEnum = IntEnum('ShadingPatternEnum', _dict)
    build_enum()
else:
