...
if not TYPE_CHECKING and _DYNAMIC:
    def _dynamic_singleton() -> None:
        ...
        import uno
        global theJobExecutor

        def _singleton_init():
            ctx = uno.getComponentContext()
            key = '/singletons/com.sun.star.task.theJobExecutor/service'
            ...
            if not hasattr(singleton, '__ooo_ns__'):
                setattr(singleton, '__ooo_ns__', 'com.sun.star.task')
                setattr(singleton, '__ooo_full_ns__', 'com.sun.star.task.theJobExecutor')
                setattr(singleton, '__ooo_type_name__', 'singleton')
            return singleton
        theJobExecutor = _singleton_init
    _dynamic_singleton()
else:
