...
if (not TYPE_CHECKING) and UNO_RUNTIME and UNO_ENVIRONMENT:
    ...
    def _get_class():
        orig_init = None
        def init(self, Host = UNO_NONE, Actions = UNO_NONE):
            if getattr(Host, "__class__", None) == self.__class__:
                orig_init(self, Host)
                return
            else:
                orig_init(self)
            if not Host is UNO_NONE:
                if getattr(self, 'Host') != Host:
        ...
        struct.__ooo_ns__ = 'com.sun.star.connection'
