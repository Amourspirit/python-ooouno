...
class Position3D(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``Position3D`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``Position3D``
        ...
        key_order = ('PositionX', 'PositionY', 'PositionZ')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], Position3D):
                oth = args[0]
        ...
        if  arg_len > len(key_order):
            raise ValueError("Position3D.__init__() To many parameters")
