...
if (not TYPE_CHECKING) and UNO_RUNTIME and UNO_ENVIRONMENT:
    ...
    def _get_class():
        orig_init = None
        def init(self, Options = UNO_NONE, Object = UNO_NONE):
            if getattr(Options, "__class__", None) == self.__class__:
                orig_init(self, Options)
                return
            else:
                orig_init(self)
            if not Options is UNO_NONE:
                if getattr(self, 'Options') != Options:
