...
if (not TYPE_CHECKING) and UNO_RUNTIME and UNO_ENVIRONMENT:
    ...
    def _get_class():
        orig_init = None
        def init(self, eError = UNO_NONE, **kwargs):
            if getattr(eError, "__class__", None) == self.__class__:
                orig_init(self, eError)
                return
            else:
                orig_init(self)
            if not eError is UNO_NONE:
                if getattr(self, 'eError') != eError:
                    setattr(self, 'eError', eError)
