...
if (not TYPE_CHECKING) and UNO_RUNTIME and UNO_ENVIRONMENT:
    ...
    def _get_class():
        orig_init = None
        def init(self, Operation = UNO_NONE, SourceURL = UNO_NONE, TargetURL = UNO_NONE, NewTitle = UNO_NONE, NameClash = UNO_NONE):
            if getattr(Operation, "__class__", None) == self.__class__:
                orig_init(self, Operation)
                return
            else:
                orig_init(self)
            if not Operation is UNO_NONE:
                if getattr(self, 'Operation') != Operation:
    ...
    GlobalTransferCommandArgument = _get_class()
