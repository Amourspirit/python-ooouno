# coding: utf-8
#
# Copyright 2022 :Barry-Thomas-Paul: Moss
#
# Licensed under the Apache License, Version 2.0 (the "License")
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http: // www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#
# Struct Class
# this is a auto generated file generated by Cheetah
# Namespace: com.sun.star.drawing
# Libre Office Version: 7.2
import os
import typing
if typing.TYPE_CHECKING:
    from .position3_d import Position3D as Position3D_bddc0bc0
from ooo_uno.oenv import UNO_ENVIRONMENT
if (not typing.TYPE_CHECKING) and UNO_ENVIRONMENT:
    from ooo_uno.helper import uno_helper


class BoundVolume(object):
    """
    Struct Class

    specifies a three-dimensional boundary volume with two positions.

    See Also:
        `API BoundVolume <https://api.libreoffice.org/docs/idl/ref/structcom_1_1sun_1_1star_1_1drawing_1_1BoundVolume.html>`_


    Note:
        | At runtime BoundVolume will be an actual uno struct however can seamlessly be treated as a regualr class.
        | At design time a class is presumed. This allows for better typings.
        | BoundVolume is a callable and can be treatead as a class and create instances.
    """

    def __init__(self, max: 'typing.Optional[Position3D_bddc0bc0]' = None, min: 'typing.Optional[Position3D_bddc0bc0]' = None):
        self._max = max
        self._min = min

    @property
    def max(self) -> 'Position3D_bddc0bc0':
        """
        this is the maximum position inside the boundary volume.
        """
        return self._max
    
    @max.setter
    def max(self, value: 'Position3D_bddc0bc0') -> None:
        self._max = value

    @property
    def min(self) -> 'Position3D_bddc0bc0':
        """
        this is the minimum position inside the boundary volume.
        """
        return self._min
    
    @min.setter
    def min(self, value: 'Position3D_bddc0bc0') -> None:
        self._min = value

def _dynamic_struct() -> None:
    # Dynamically create uno struct using uno
    global BoundVolume
    ignore = os.environ.get('ooouno_ignore_runtime', 'False')
    if ignore == 'True':
        return
    if (not typing.TYPE_CHECKING) and UNO_ENVIRONMENT:
        order = ('max', 'min')
        def _struct_init(*args, **kwargs):
            struct = uno_helper.create_uno_struct('com.sun.star.drawing.BoundVolume')
            for i, arg in enumerate(args):
                if arg is None:
                    continue
                setattr(struct, order[i], arg)
            for k, v in kwargs.items():
                setattr(struct, k, v)
            return struct
        BoundVolume = _struct_init

if (not typing.TYPE_CHECKING) and UNO_ENVIRONMENT:
    _dynamic_struct()

__all__ = ['BoundVolume']
