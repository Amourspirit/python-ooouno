...
class DragSourceDropEvent(DragSourceEvent_8ccf115c):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``DragSourceDropEvent`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``DragSourceDropEvent``
        ...
        self._drop_success = None

        key_order = ('DropAction', 'DropSuccess')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], DragSourceDropEvent):
                oth = args[0]
