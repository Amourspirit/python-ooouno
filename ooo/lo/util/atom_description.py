...
class AtomDescription(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Other Arguments:
            First positinal arg can be another ``AtomDescription`` instance,
                if it is the only argument passed to contructor;
                Otherwise, postitional arguments are treated as ``AtomDescription``
        ...
        key_order = ('atom', 'description')
        arg_len = len(args)
        if arg_len == 1:
            if isinstance(args[0], AtomDescription):
                oth = args[0]
        ...
        if  arg_len > len(key_order):
            raise ValueError("AtomDescription.__init__() To many parameters")
